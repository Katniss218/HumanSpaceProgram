        ...
        public static AudioMixerGroup GetAudioMixerGroup( this AudioChannel channel )
        {
            ...
            switch( channel )
            {
                case AudioChannel.Main_3D:
                    group = AudioManager.AudioMixer.FindMatchingGroups( "3D Main" )[0];
                    break;
                case AudioChannel.Ambient_3D:
                    group = AudioManager.AudioMixer.FindMatchingGroups( "3D Ambient" )[0];
                    break;
                case AudioChannel.Ambient_2D:
                    group = AudioManager.AudioMixer.FindMatchingGroups( "2D Ambient" )[0];
                    break;

                case AudioChannel.Main_2D:
                    group = AudioManager.AudioMixer.FindMatchingGroups( "2D Main" )[0];
                    break;
                case AudioChannel.Music:
                    group = AudioManager.AudioMixer.FindMatchingGroups( "Music" )[0];
                    break;
                case AudioChannel.UI:
                    group = AudioManager.AudioMixer.FindMatchingGroups( "UI" )[0];
                    break;
            }
