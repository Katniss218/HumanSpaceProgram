...
namespace HSP.Vanilla.Scenes.GameplayScene
{
    ...
    {
        [HSPEventListener( HSPEvent_STARTUP_GAMEPLAY.ID, HSPEvent.NAMESPACE_HSP + ".add_timescale_input_controller" )]
        ...
        [HSPEventListener( HSPEvent_STARTUP_GAMEPLAY.ID, HSPEvent.NAMESPACE_HSP + ".add_vessel_manager" )]
        private static void AddVesselManager()
        {
            GameplaySceneManager.Instance.gameObject.AddComponent<VesselManager>();
        }

        [HSPEventListener( HSPEvent_STARTUP_GAMEPLAY.ID, HSPEvent.NAMESPACE_HSP + ".add_celestial_body_manager" )]
        private static void AddCelestialBodyManager()
        {
            GameplaySceneManager.Instance.gameObject.AddComponent<CelestialBodyManager>();
        }

        [HSPEventListener( HSPEvent_STARTUP_GAMEPLAY.ID, HSPEvent.NAMESPACE_HSP + ".add_active_object_manager" )]
        private static void AddActiveObjectManager()
        {
            GameplaySceneManager.Instance.gameObject.AddComponent<ActiveObjectManager>();
        }
        
        [HSPEventListener( HSPEvent_STARTUP_GAMEPLAY.ID, HSPEvent.NAMESPACE_HSP + ".add_gameplay_scene_tool_manager" )]
        ...
        [HSPEventListener( HSPEvent_STARTUP_GAMEPLAY.ID, HSPEvent.NAMESPACE_HSP + ".add_scene_reference_frame_manager" )]
        ...
        [HSPEventListener( HSPEvent_STARTUP_GAMEPLAY.ID, "add_atmosphere_renderer", After = new[] { "vanilla.gameplayscene_camera" } )]
