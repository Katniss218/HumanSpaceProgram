...
{
    public sealed class UIContextMenuCanvas : UICanvas
    {


        public static new UIContextMenuCanvas Create( Scene scene, string id )
        {
            var x = Create<UIContextMenuCanvas>( scene, id );

            x.canvas.renderMode = RenderMode.ScreenSpaceOverlay;
            x.canvas.pixelPerfect = false;
            x.canvas.sortingOrder = 30;
