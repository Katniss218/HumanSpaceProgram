...
#warning TODO - hyperbolic (escaping) orbits.
        ...
        public void RoundTrip___EarthWeirdCase___IsCorrect()
        {
            ...
            TrajectoryBodyState source = new TrajectoryBodyState( new Vector3Dbl( 150000000000, 20000, 0 ), new Vector3Dbl( -55, 29749.1543788567, 0 ), Vector3Dbl.zero, 5.97e24 );
            target.SetCurrentState( source );

            // Act
            TrajectoryBodyState targetState = target.GetCurrentState();

            Assert.That( targetState.AbsolutePosition, Is.EqualTo( source.AbsolutePosition ).Using( posApproxComparer ) );
        ...
        [Test]
        public void KeplerianFollowsNewtonian()
        {
            FixedOrbit parent = new FixedOrbit( 0, Vector3Dbl.zero, QuaternionDbl.identity, 1.989e30 );
            TrajectoryBodyState state = new TrajectoryBodyState( new Vector3Dbl( 150_000_000_000, 0, 0 ), new Vector3Dbl( 0, 29750, 0 ), Vector3Dbl.zero, 5.97e24 );
