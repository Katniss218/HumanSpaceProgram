...
namespace HSP.Vanilla.UI.Scenes.GameplayScene
{
    public static class OnStartup
    {
        ...
        [HSPEventListener( HSPEvent_ON_TIMELINE_NEW_ERROR.ID, SHOW_GAMEPLAY_LOAD_MESSAGE )]
        private static void errorSc( HSPEvent_ON_TIMELINE_NEW_ERROR.EventData e )
        {
            HSPSceneManager.ReplaceForegroundScene<MainMenuSceneM>( onAfterLoaded: () =>
            {
                UICanvas canvas = MainMenuSceneM.Instance.GetWindowCanvas();
                var errorMessages = e.data.GetMessages( LogType.Error );
                canvas.AddConfirmWindow( "Error", $"An error occurred while starting the scenario:\n\n{errorMessages.FirstOrDefault()}\n See the log file for the full list.", null );
            } );
        }
        ...
        private static void success( HSPEvent_ON_TIMELINE_LOAD_SUCCESS.EventData e )
        {
            ...
            {
                UICanvas canvas = HSPSceneManager.ForegroundScene.GetWindowCanvas();
